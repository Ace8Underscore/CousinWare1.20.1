...
public class CFontRenderer extends CFont {
    ...
    public float drawString(String text, double x, double y, int color, boolean shadow) {
        ...
        if (render) {
            ...
            GL11.glColor4d((color >> 16 & 0xFF) / 255.0F, (color >> 8 & 0xFF) / 255.0F, (color & 0xFF) / 255.0F, alpha);
            int size = text.length();
            //GL11.enableTexture2D();
            //GlStateManager._bindTexture(tex.getGlId());
            GlStateManager._bindTexture(tex.getGlId());

            GL11.glBindTexture(GL11.GL_TEXTURE_2D, tex.getGlId());
            for (int i = 0; i < size; i++) {
                ...
                if ((character == '\u00A7') && (i < size)) {
                    ...
                    if (colorIndex < 16) {
                        ...
                        GlStateManager._clearColor((colorcode >> 16 & 0xFF) / 255.0F, (colorcode >> 8 & 0xFF) / 255.0F, (colorcode & 0xFF) / 255.0F, alpha);
                    } else if (colorIndex == 16) randomCase = true;
                    else if (colorIndex == 17) {
                        bold = true;
                        if (italic) {
                            GlStateManager._bindTexture(texItalicBold.getGlId());
                            // GL11.glBindTexture(GL11.GL_TEXTURE_2D,
                            // texItalicBold.getGlId());
                            currentData = this.boldItalicChars;
                        } else {
                            GlStateManager._bindTexture(texBold.getGlId());
                            // GL11.glBindTexture(GL11.GL_TEXTURE_2D,
                            // texBold.getGlId());
                        ...
                        if (bold) {
                            ...
                            GlStateManager._bindTexture(texItalic.getGlId());
                            // GL11.glBindTexture(GL11.GL_TEXTURE_2D,
                            // texItalic.getGlId());
                        ...
                        underline = false;
                        strikethrough = false;
                        GlStateManager._clearColor((color >> 16 & 0xFF) / 255.0F, (color >> 8 & 0xFF) / 255.0F, (color & 0xFF) / 255.0F, alpha);
                        GlStateManager._bindTexture(tex.getGlId());
                        // GL11.glBindTexture(GL11.GL_TEXTURE_2D,
                        // tex.getGlId());
                        currentData = this.charData;
